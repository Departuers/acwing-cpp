...
如果前一颗能量珠的头标记为m，尾标记为r，后一颗能量珠的头标记为 r，
尾标记为 n，则聚合后释放的能量为 m*r*n（Mars单位），
 ...
 * f[1,5]是最大值,答案
 * 
 * f[L,R] 所有将[L,R]合并成一个矩阵(珠子)的方式,
 * 属性max
 * 状态计算:
 * 划分依据,考虑分界线
 * L+1,L+2...R-2 R-1
 * 考虑分界线是K
 * f[L,R]=f[L,K]+f[K,R]+w[L]*w[K]*w[R] 取max
 * 把长度为3的区间合并为长度为2的区间,把中间点去掉
 * 如果是环形,枚举所有长度为n+1的区间
 * 
 * */
const int N=210;
...
int main(int argc, char const *argv[])
{
    ...
    int res=0;
    for(int i=1;i<=n;i++)res=max(res,f[i][i+n]);//枚举所有长度为n+1的区间
    cout<<res<<endl;
    return 0;
}